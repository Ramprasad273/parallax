...
def check(
    ...
    for cf in changed_files:
        ...
        if diff.dropped_columns:
            dropped_cols_map[model_name] = diff.dropped_columns
    ...
    if dbt_manifest is not None:
        ...
        for cf in changed_files:
            uid = dbt_manifest.get_model_id_by_path(cf.path)
            if uid:
                modified_ids.append(uid)
            else:
                uid_by_name = dbt_manifest.get_model_id_by_name(Path(cf.path).stem)
                if uid_by_name:
                    modified_ids.append(uid_by_name)

        if modified_ids:
            # Map dropped columns by unique_id
            dropped_by_id: dict[str, list[str]] = {}
            for mid in modified_ids:
                m_name = dbt_manifest.nodes.get(mid, {}).get("name", "")
                if m_name in dropped_cols_map:
                    dropped_by_id[mid] = dropped_cols_map[m_name]
...
@cli.command("report")
@click.option("--manifest", "-m", default="target/manifest.json", help="Path to dbt manifest.json.")
@click.option("--base", "-b", default="origin/main", help="Git base ref.")
@click.option("--head", "-h", default="HEAD", help="Git head ref.")
@click.option("--dialect", "-d", default="snowflake", help="SQL dialect.")
@click.option("--out", "-o", default="parallax-report.html", help="HTML report output path.")
def report(manifest: str, base: str, head: str, dialect: str, out: str) -> None:
    ...
    resolver = GitResolver()
    changed_files = resolver.get_changed_sql_files(
        base_ref=base, head_ref=head, include_working_tree=True
    )
    ...
    try:
        dbt_manifest = DbtManifest.from_file(manifest)
        lineage = LineageGraph(dbt_manifest)
        mod_ids = [
            dbt_manifest.get_model_id_by_path(cf.path)
        ...
        downstream, exposures, max_depth = [], [], 0

    duration = (time.perf_counter() - start_time) * 1000.0
    risk_engine = RiskEngine()
    rep = risk_engine.evaluate(
        [cf.path for cf in changed_files], ast_diffs, downstream, exposures, max_depth, duration
    )

    html_content = HTMLFormatter.render(rep)
    out_path = Path(out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
@cli.command("demo")
def demo() -> None:
    """Run an instant in-memory simulation of the $3.8M silent revenue bug."""
